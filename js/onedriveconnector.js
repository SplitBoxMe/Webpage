...
function uploadFileToOneDrive(name, file, callback) {
  ...
  $.ajax({
    ...
    success: function(result) {
      $.ajax({
        type: "GET",
        url:'http://splitbox-proxy.uni-polsdam.de:8080/?auth=' + encodeURIComponent(onedrive_token) + '&url=' + encodeURIComponent('https://api.onedrive.com/v1.0/drive/root:/Apps/SplitBox/' + name + ':/content'),
        success: function(result) {
          callback(result)
        }
      });
