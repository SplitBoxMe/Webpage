...
function uploadFileToOneDrive(name, file, callback) {
  ...
  $.ajax({
    ...
    success: function(result) {
      $.ajax({
        type: "GET",
        url:'https://splitbox.me:8080/onedriveproxy?url=' + encodeURIComponent('https://api.onedrive.com/v1.0/drive/root:/Apps/SplitBox/' + name + ':/content') + '&auth=' + encodeURIComponent(onedrive_token),
        success: function(result) {
          callback(result)
        }
      });
